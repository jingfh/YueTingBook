...
var __that = null;
//录音
var _openRecording = null;
var _shutRecording = null;

// 初始化函数
function init(that, hotKeys, tipKeys, searchFunction, goBackFunction, __openRecording, __shutRecording, openRecordingdis, shutRecordingdis) {

  __that = that;
  __tipKeys = tipKeys;
  __searchFunction = searchFunction;
  __goBackFunction = goBackFunction;
  _openRecording = __openRecording;
  _shutRecording = __shutRecording;

  var temData = {};
  temData.openRecordingdis = openRecordingdis;
  temData.shutRecordingdis = shutRecordingdis;
  var barHeight = 43;
  var view = {
    barHeight: barHeight
  }
  ...
  wx.getSystemInfo({
    success: function (res) {
      ...
      view.seachHeight = wHeight - barHeight - 66;
      temData.view = view;
      __that.setData({
...
// shutRecording
function shutRecording() {
  _shutRecording();
}
//changeRecordBgImg
function changeRecordBgImg(){
  var temData = __that.data.wxSearchData;
  if(temData.openRecordingdis=="block"){
    temData.openRecordingdis="none";
    temData.shutRecordingdis="block";
  }else{
    temData.openRecordingdis = "block";
    temData.shutRecordingdis = "none";
  }
  __that.setData({
    wxSearchData: temData
  });
...
// 通过录音对搜索框进行输入
function wxSearchInputByRecord(inputValue) {
  var temData = __that.data.wxSearchData;
  var tipKeys = [];
  if (inputValue && inputValue.length > 0) {
    for (var i = 0; i < __tipKeys.length; i++) {
  ...
  temData.value = temData.value+inputValue;
  temData.tipKeys = tipKeys;
  __that.setData({
    wxSearchData: temData
  });
...
module.exports = {
  ...
  wxSearchInputByRecord: wxSearchInputByRecord,// 通过录音输入
  ...
  changeRecordBgImg: changeRecordBgImg, //change录音按钮背景
}